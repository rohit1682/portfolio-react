export const skillCategories = [
  {
    category: "Primary — Languages",
    items: [
      { name: "JavaScript", level: 92 },
      { name: "TypeScript", level: 80 },
      { name: "Java", level: 88 },
      { name: "Python", level: 78 },
    ],
  },
  {
    category: "Frontend",
    items: [
      { name: "React.js", level: 88 },
      { name: "Next.js", level: 75 },
      { name: "Angular", level: 72 },
      { name: "HTML / Custom CSS", level: 95 },
      { name: "Tailwind CSS", level: 85 },
      { name: "Bootstrap", level: 88 },
      { name: "Figma / SEO", level: 75 },
    ],
  },
  {
    category: "Backend & Databases",
    items: [
      { name: "Node.js", level: 85 },
      { name: "Express.js", level: 85 },
      { name: "NestJS", level: 78 },
      { name: "GraphQL", level: 80 },
      { name: "MongoDB", level: 80 },
      { name: "PHP / PL-SQL", level: 75 },
      { name: "Django REST API", level: 72 },
      { name: "MySQL", level: 82 },
      { name: "REST APIs", level: 88 },
    ],
  },
  {
    category: "Tools & Methodologies",
    items: [
      { name: "Git & GitHub", level: 90 },
      { name: "JIRA (Agile / Scrum)", level: 82 },
      { name: "JEST Unit Testing", level: 78 },
      { name: "Postman", level: 85 },
      { name: "AWS (S3, DynamoDB)", level: 70 },
      { name: "Azure", level: 72 },
      { name: "Docker", level: 68 },
      { name: "VS Code", level: 95 },
    ],
  },
  {
    category: "AI & GenAI",
    items: [
      { name: "Prompt Engineering", level: 90 },
      { name: "Claude (Anthropic)", level: 88 },
      { name: "ChatGPT / GPT APIs", level: 86 },
      { name: "GitHub Copilot", level: 88 },
      { name: "Generative AI Fundamentals", level: 85 },
      { name: "Machine Learning", level: 78 },
      { name: "LLM App Integration", level: 80 },
      { name: "Custom Agents", level: 82 },
      { name: "Automation", level: 85 },
      { name: "AI-Driven Problem Solving", level: 82 },
    ],
  },
  {
    category: "E-commerce & Platforms",
    items: [
      { name: "BigCommerce", level: 80 },
      { name: "MACH Architecture", level: 78 },
      { name: "Listrak", level: 75 },
      { name: "BazaarVoice", level: 72 },
      { name: "OrderDynamics", level: 72 },
      { name: "Noibu", level: 70 },
    ],
  },
  {
    category: "Languages & Soft Skills",
    items: [
      { name: "English (Fluent)", level: 95 },
      { name: "Hindi (Native)", level: 100 },
      { name: "Bengali (Native)", level: 100 },
      { name: "German (Communicative)", level: 65 },
      { name: "Communication & Public Speaking", level: 88 },
      { name: "Leadership & Team Building", level: 85 },
    ],
  },
];

export const skillTags = {
  "Primary Languages": ["JavaScript", "TypeScript", "Java", "Python"],
  "Frontend": ["React.js", "Next.js", "Angular", "HTML5", "Custom CSS", "Tailwind CSS", "Bootstrap", "Figma", "SEO Optimization"],
  "Backend": ["Node.js", "Express.js", "NestJS", "GraphQL", "MongoDB", "PHP", "PL/SQL", "Django REST API", "REST APIs"],
  "Cloud & DevOps": ["AWS S3", "AWS DynamoDB", "Azure", "Docker", "Ubuntu", "VirtualBox VM"],
  "E-commerce & Platforms": ["BigCommerce", "MACH Architecture", "Listrak", "BazaarVoice", "Noibu", "OrderDynamics"],
  "Testing & Tools": ["JEST", "Postman", "MongoDB Compass", "Botpress", "GitHub", "VS Code"],
  "Methodologies": ["Agile", "Scrum", "JIRA", "OOP", "Design Patterns", "Automation"],
  "AI & GenAI": [
    "Prompt Engineering",
    "Claude (Anthropic)",
    "Claude Architect",
    "ChatGPT",
    "GPT-4 / GPT APIs",
    "GitHub Copilot",
    "Generative AI",
    "Machine Learning",
    "AI-Driven Problem Solving",
    "LLM Integration",
    "RAG Basics",
    "AI Agents",
    "MCP (Model Context Protocol)",
    "AI-Assisted Development",
    "Custom Agents",
    "Anthropic Certified",
  ],
  "Secondary Languages": ["C", "C++"],
  "Documentation": ["MS Excel", "MS PowerPoint", "Google Docs", "Google Sheets", "Google Slides"],
  "Soft Skills": ["Problem Solving", "Communication", "Planning & Organizing", "Adaptability", "Team Building", "Event Management"],
};
